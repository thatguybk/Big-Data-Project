...
st.markdown("The graph above compares the cost of generating the net amount of electricity generated by natural gas, using natural gas vs using solar. The cost of generating that amount of electricity by only using natural gas is significantly lower than the cost of only using solar. This is likely because implementing solar power in the form of Photovoltaic cells, requires a large amount of money for initial investment and infrastructure. Solar Panels also require more frequent maintenance, which can be expensive. It can be seen that over the years, the cost of generating energy using solar has gradually decreased. This is due to the increased production and avaliability of solar panels. If one desired to replace natural gas generation with solar, the optimal year to do so is 2022 as it is the year with the lowest cost of generating energy using solar and the difference between the cost of generating energy using natural gas and solar is the smallest.")
